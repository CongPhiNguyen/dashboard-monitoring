const URL_SYSTEM_V1 = "http://localhost:5050/api/v1";
const URL = {
  URL_GET_ALL_DATA: URL_SYSTEM_V1 + "/vui-coins/get-all-data",
  URL_ADMIN_LOGIN: URL_SYSTEM_V1 + "/user/login",
  URL_ADMIN_REFRESH: URL_SYSTEM_V1 + "/user/refresh",
  URL_GET_DATA_WEEK: URL_SYSTEM_V1 + "/vui-coins/get-data-week",
  URL_GET_DATA_DAY: URL_SYSTEM_V1 + "/vui-coins/get-data-day",
  URL_GET_DATA_HOURS: URL_SYSTEM_V1 + "/vui-coins/get-data-hours",
};

export default URL;
